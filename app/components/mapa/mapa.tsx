...
function MyMap() {
  ...
  return (
    <>
          ...
          return (
            <CircleMarker
              ...
              color={
                ...
                (venue.check_ins?.length || 0) < 5 ? '#f59e0b' : '#ef4444'
              }
              fillColor={
                eventStatus === 'active' ? '#d8b4fe' :
                eventStatus === 'soon'   ? '#fdba74' :
                venue.check_ins?.length === 0 ? '#6ee7b7' :
                (venue.check_ins?.length || 0) < 5 ? '#fcd34d' : '#fca5a5'
              }
              fillOpacity={0.85}
      ...
      {selectedVenue && (
        <>
          <div className="fixed inset-0 bg-black/40 z-[989]" onClick={closeModal} />
          
          <div className="fixed bottom-0 left-0 right-0 z-[1002] animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="bg-gray-900 rounded-t-3xl max-w-2xl mx-auto">
              <div className="relative h-64 rounded-t-3xl overflow-hidden">
                ...
                <button onClick={closeModal} className="absolute top-4 right-4 bg-black bg-opacity-50 rounded-full p-2 hover:bg-opacity-70 transition">
                ...
                <div className="absolute top-4 left-4 text-white text-xs font-bold px-3 py-1 rounded-full" style={{
                  backgroundColor:
                    ...
                    (selectedVenue.check_ins?.length || 0) < 5 ? '#f59e0b' : '#ef4444'
                }}>
